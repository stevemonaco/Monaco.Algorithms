...
{
    public class PathTrie<T> : IPathTrie<T>
    {
        private IPathTrieNode<T> root = new PathTrieNode<T>($"{nameof(PathTrie<T>)}.RootNode", default);
        ...
        public void Add(string path, T value)
        {
            ...
            IPathTrieNode<T> nodeVisitor = root;
        ...
        public bool TryGetNode(string path, out IPathTrieNode<T> node)
        {
            if (string.IsNullOrWhiteSpace(path))
            ...
            IPathTrieNode<T> nextNode = default;

            foreach (var name in nodeNames)
            {
                if (!nodeVisitor.TryGetChild(name, out nextNode))
                {
                    node = default(PathTrieNode<T>);
                    return false;
                }
        ...
        /// </remarks>
        public IEnumerable<IPathTrieNode<T>> EnumerateDepthFirst() => root.DescendantsDepthFirst();

        public IEnumerable<IPathTrieNode<T>> EnumerateBreadthFirst() => root.DescendantsDepthFirst();
